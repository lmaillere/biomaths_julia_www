...
Et on définit une fonction qui définit le problème de simulation, l'intègre et produit un graphique de la simulation contre le temps :
```{julia}
function plot_rma(mod, u0::SVector{2}, p::ParRma, pt::ParTime)
    (; tspan, tstep) = pt
    # define and solve simulation problem
    prob_rma = ODEProblem(mod, u0, tspan, p; saveat = tstep)
...
@time plot_rma(mod_rma, u0, p_rma, time)
```
::: {.callout-note}
La macro `@time` permet de calculer le temps mis pour calculer la commande qui la suit.
:::

Au premier appel de la fonction `plot_rma()`, il y a un temps de précompilation de la fonction qui amène à un plot en 1.33s sur ma machine.

Des appels après précompilation (en changeant les paramètres !) sont de l'ordre de deux ordres de grandeurs plus rapides :

```{julia}
#| output: true
@time plot_rma(mod_rma, u0, p_rma2, time)
```
