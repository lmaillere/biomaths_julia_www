...
La façon la plus simple d'installer `Julia` est d'utiliser [juliaup](https://github.com/JuliaLang/juliaup). Installez `curl` sur votre système^[`sudo apt install curl` sur Ubuntu].
...
Puis, si vous souhaitez utiliser `Julia` dans un notebook `jupyter`, installez le package `IJulia` depuis Julia comme suit^[le profil utilisé doit être administrateur, et si il est tout nouvellement créé un redémarrage peut s'avérer nécessaire si vous rencontrez une erreur à l'exécution de `jupyterlab()`]:
```{julia}
#| eval: false
...
Une [extension `Julia`](https://www.julia-vscode.org/docs/stable/setup/) est enfin disponible pour VScode/VScodium.
