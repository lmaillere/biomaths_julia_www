...
Pour simuler ce modèle, c'est à dire intégrer numériquement les solutions au problème de Cauchy correspondant à l'@eq-malthus avec $x(0)=x_0\geq0$, nous allons utiliser les routines de `DifferentialEquations.jl` avec une méthode utilisant des tableaux statiques pour la rapidité d'exécution (package `StaticArrays.jl`). Nous utiliserons aussi le package `Plots.jl` pour les représentations graphiques et `LatexStrings.jl` pour les mathématiques sur les graphiques.

```{julia}
using Plots, DifferentialEquations, StaticArrays, LaTeXStrings
```
