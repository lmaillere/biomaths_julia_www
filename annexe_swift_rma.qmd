...
```{julia}
#| output: true
# ParRma objects are immutable (due to the struct definition)
...
La fonction prend pour argument les objets condition initiale, paramètre et paramètre de temps, et fait appel à la fonction `sim_rma()` avec les méthodes `final = true` (pour les transitoires) et `final = false` (pour les extremas du cycle limite).
...
function cy_rma(iniv::IniV, p::ParRma, pt::ParTime; Kmax::Real = 8.0, Kstep::Real = 0.01)
    ...
    for (i, Kh) in enumerate(Krgh)
        ...
        sol_cyc = sim_rma(inivtr, prmabif, pt)
    ...
    cycle = (Krgh, ycmin, ycmax)
    return cycle
end

@time cy_rma(iniv, prma, ptime);
```

```{julia}
#| output: true
#| eval: true
@time cy_rma(iniv, prma2, ptime);
```
