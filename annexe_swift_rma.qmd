...
@kwdef struct ParTime
    tspan::Tuple{Real, Real} = (0.0, 60.0)
    tstep::Real = 0.1
end
...
```{julia}
#| output: true
function cy_rma(p::ParRma, pt::ParTime; Kmax::Real = 8.0, Kstep::Real = 0.01)
    ...
    for (i, Kb) in enumerate(K_p3)
        ...
        uf = sim_rma(u0, p_rmabif, p_ttrans; final = true)[:,1]

        # start from end of transient, simulate cycle
        sol_cyc = sim_rma(uf, p_rmabif, pt)
